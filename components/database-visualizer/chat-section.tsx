    ...
    <div className="h-full flex flex-col">
      <ScrollArea className="flex-grow mb-4 p-4">
        {messages.map((message, index) => (
          <div key={index} className={`mb-2 p-2 ${message.role === 'user' ? 'text-right' : 'text-left'}`}>
            <span className={`inline-block p-2 rounded-lg ${message.role === 'user' ? 'bg-blue-600 text-white' : 'bg-gray-600 text-white'}`}>
              {message.content}
            </span>
          </div>
        ))}
      ...
      <div className="flex p-4 bg-gray-700">
        <Input
          ...
          className="flex-grow mr-2 bg-gray-600 text-white border-gray-500"
